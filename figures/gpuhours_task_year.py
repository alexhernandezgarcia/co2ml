"""
This script plots the GPU hours of each model in the data set, disaggregating
per task and year.
"""
import os
...
# -----  Constants  -----
# -----------------------
dict_trainingtime = {"1,209,600": 1209600}
hue_order = [
...
if __name__ == "__main__":
    ...
    # Store args
    output_yml = output_dir / "gpuhours_task_year.yml"
    ...
    # Get GPU hours
    df["n_devices"] = df.loc[:, ["Number of GPUs", "Number of TPUs"]].max(axis=1)
    df.replace({"Training time [s]": dict_trainingtime}, regex=True, inplace=True)
    df["GPU/TPU time [s]"] = df["Training time [s]"].astype(int) * df["n_devices"]

    fig = plot_data(df, estimator=estimator, args=args)

    # Save figure
    output_fig = output_dir / "gpuhours_task_year.png"
